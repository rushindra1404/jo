...
export const DashboardScreen: React.FC = () => {
  ...
  if (examReadinessScore >= 85) {
    ...
    readinessColor = 'text-orange-500 bg-orange-50 dark:bg-orange-955/20';
  ...
  const currentStreak = stats.streakDays || 1;
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 pb-24 pt-4 space-y-6">
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-premium space-y-4">
        <h3 className="text-xs font-extrabold uppercase text-slate-404 dark:text-slate-555 tracking-wider">
          Overall Progress
        </h3>
        <div className="flex items-center justify-between gap-4">
          <div className="relative w-24 h-24 shrink-0 flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              ...
              <span className="text-[8px] text-slate-404 uppercase tracking-widest font-bold">Done</span>
            </div>
          </div>
          ...
          <div className="flex-1 grid grid-cols-2 gap-2 text-center text-xs font-bold">
            <div className="bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl">
              ...
              <span className="text-[9px] text-slate-404 font-medium">Available</span>
            </div>
            <div className="bg-slate-50 dark:bg-slate-955 p-2.5 rounded-xl">
              <span className="block text-slate-800 dark:text-slate-100 font-sans text-base">{attemptedQuestions}</span>
              <span className="text-[9px] text-slate-404 font-medium">Attempted</span>
            </div>
            <div className="bg-slate-50 dark:bg-slate-955 p-2.5 rounded-xl col-span-2">
              <span className="block text-slate-800 dark:text-slate-100 font-sans text-base">{remainingQuestions}</span>
              <span className="text-[9px] text-slate-404 font-medium">Remaining Questions</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={`bg-white dark:bg-slate-900 border rounded-3xl p-5 shadow-premium space-y-4 ${readinessBorder}`}>
        ...
        <div className="text-center space-y-1 py-1">
          <span className="text-[10px] font-extrabold uppercase text-slate-404 tracking-wider">Score Meter</span>
          <p className="text-5xl font-black text-cyan-600 dark:text-cyan-400 font-sans">{examReadinessScore}%</p>
        </div>

        <div className="bg-slate-50 dark:bg-slate-955 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl text-xs space-y-1 font-semibold leading-relaxed text-slate-700 dark:text-slate-300">
          <strong className="text-[10px] uppercase font-black tracking-wider text-slate-404 block mb-0.5">
            Revision Advisor recommendation:
          </strong>
          <p className="font-bold text-slate-805 dark:text-slate-100 text-sm">{recommendation}</p>
        </div>
      </section>
      ...
      <div className="space-y-6">
        <section className="space-y-3">
          <h3 className="text-xs font-extrabold uppercase text-slate-404 tracking-wider">
            Weak Chapters (Below 70%)
          </h3>
          {weakChapters.length === 0 ? (
            <div className="p-5 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl text-center text-xs text-slate-404">
              No weak chapters logged yet. Study more chapters to run advisor evaluation.
            </div>
          ) : (
            <div className="space-y-2">
              {weakChapters.slice(0, 3).map(ch => (
                <div key={ch.id} className="bg-red-50 dark:bg-red-955/20 border border-red-200 dark:border-red-900 rounded-2xl p-4 flex items-center justify-between">
                  <div className="flex-1 pr-3">
                    ...
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-202 mt-1 line-clamp-1">{ch.title}</h4>
          ...
          {strongChapters.length === 0 ? (
            ...
            <div className="space-y-2">
              {strongChapters.slice(0, 3).map(ch => (
                <div key={ch.id} className="bg-emerald-50 dark:bg-emerald-955/20 border border-emerald-200 dark:border-emerald-900 rounded-2xl p-4 flex items-center justify-between">
                  <div className="flex-1 pr-3">
                    <div className="flex items-center gap-2 text-[10px] font-bold text-emerald-700 dark:text-emerald-450">
                      <span>{ch.material.toUpperCase()} • Chapter {ch.num}</span>
                      <span>({ch.attempted} Attempted)</span>
                    </div>
                    <h4 className="text-sm font-bold text-slate-850 dark:text-slate-202 mt-1 line-clamp-1">{ch.title}</h4>
          ...
          >
            <div className="flex-1 pr-2 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-extrabold text-emerald-605 dark:text-emerald-450 bg-emerald-50 dark:bg-emerald-955/20 px-2 py-0.5 rounded-lg">
                  {icaAccuracy}% Acc
                </span>
              </div>
              ...
              <div className="flex justify-between items-center text-[10px] text-slate-404 font-bold uppercase">
          ...
          >
            <div className="flex-1 pr-2 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-extrabold text-indigo-650 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-955/20 px-2 py-0.5 rounded-lg">
                  {gpoeAccuracy}% Acc
                </span>
              </div>
              <div className="w-full bg-slate-100 dark:bg-slate-955 h-2 rounded-full overflow-hidden">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2 text-center text-xs font-bold text-slate-700 dark:text-slate-300">
                <div className="p-2.5 bg-slate-50 dark:bg-slate-950 rounded-xl">
                  ...
                  <span className="text-[9px] text-slate-404 font-medium">Logged</span>
                </div>
                <div className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl">
                  <span className="block text-emerald-600 font-sans text-base">{resolvedCount}</span>
                  <span className="text-[9px] text-slate-404 font-medium">Resolved</span>
                </div>
                <div className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl">
                  <span className="block text-rose-505 font-sans text-base">{pendingCount}</span>
                  <span className="text-[9px] text-slate-404 font-medium">Pending</span>
                </div>
              </div>
              
              <div className="space-y-1">
                <div className="flex justify-between items-center text-[10px] font-bold text-slate-404">
                  <span>MISTAKES RESOLUTION</span>
                  <span className="text-emerald-600 dark:text-emerald-400">{improvementRate}% IMPROVEMENT</span>
                </div>
                <div className="w-full bg-slate-100 dark:bg-slate-955 h-2.5 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-2 text-center text-xs font-bold">
                <div className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl">
                  ...
                  <span className="text-[9px] text-slate-404 font-medium">Total</span>
                </div>
                <div className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl">
                  <span className="block text-emerald-600 font-sans text-base">{icaBookmarks}</span>
                  <span className="text-[9px] text-slate-404 font-medium">in ICA</span>
                </div>
                <div className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl">
                  <span className="block text-indigo-500 font-sans text-base">{gpoeBookmarks}</span>
                  <span className="text-[9px] text-slate-404 font-medium">in GPOE</span>
                </div>
              </div>

              {recentBookmarks.length > 0 && (
                <div className="space-y-2 pt-1 border-t border-slate-100 dark:border-slate-800">
                  <p className="text-[9px] font-extrabold uppercase text-slate-404 tracking-wider">Recently Bookmarked</p>
                  <div className="space-y-1.5">
                    {recentBookmarks.map(q => (
                      <div key={q.uniqueId} className="p-2.5 bg-slate-50 dark:bg-slate-955 rounded-xl text-xs line-clamp-1 font-semibold text-slate-700 dark:text-slate-300">
                        {q.question}
                      </div>
                    ))}
                  </div>
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-premium space-y-4">
        ...
        <div className="space-y-2">
          <h4 className="text-[11px] font-black uppercase text-slate-404 tracking-wider border-b border-slate-100 dark:border-slate-800 pb-1">
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-premium space-y-4 relative overflow-hidden">
        ...
        {progress.recentActivity.length === 0 ? (
          <div className="p-6 text-center text-xs text-slate-404 dark:text-slate-600 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl">
          ...
          <div className="space-y-5 relative">
            ...
            {progress.recentActivity.slice(0, 5).map(act => {
              const ch = act.chapterId ? getChaptersByMaterial(act.material).find(e => e.id === act.chapterId) : null;
              return (
                <div key={act.id} className="relative flex items-start gap-4 text-xs font-medium pl-8">
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex justify-between items-center text-[10px] font-bold text-slate-404">
                      ...
                      <span>{act.material.toUpperCase()} • Ch {ch?.num || 1}</span>
                    </div>
                    <h4 className="font-extrabold text-slate-805 dark:text-slate-202 text-sm leading-snug">{act.label}</h4>
                    <div className="flex items-center gap-3 mt-1.5 text-[10px] font-extrabold uppercase text-slate-404">
      ...
      <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-premium space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs font-bold">
          ...
          <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] text-slate-404 font-bold block">Total Sessions</span>
              <span className="text-base font-black text-slate-800 dark:text-slate-100 font-sans">{sessionsCount}</span>
            </div>
          </div>

          <div className="bg-slate-50 dark:bg-slate-955 p-4 rounded-2xl flex items-center gap-3">
            <div className="p-2.5 bg-cyan-50 dark:bg-cyan-955/30 text-cyan-600 dark:text-cyan-400 rounded-xl">
              <Clock size={18} />
            </div>
            <div>
              <span className="text-[10px] text-slate-404 font-bold block">Avg Duration</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-955 p-4 rounded-2xl flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] text-slate-404 font-bold block">Study Streak</span>
              <span className="text-base font-black text-slate-800 dark:text-slate-100 font-sans">{currentStreak}d</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-955 p-4 rounded-2xl flex items-center gap-3">
            ...
            <div>
              <span className="text-[10px] text-slate-404 font-bold block">Longest Streak</span>
              <span className="text-base font-black text-slate-800 dark:text-slate-100 font-sans">{bestStreak}d</span>
            </div>
          </div>

          <div className="bg-slate-50 dark:bg-slate-955 p-4 rounded-2xl flex items-center gap-3 col-span-2">
            <div className="p-2.5 bg-cyan-50 dark:bg-cyan-955/30 text-cyan-600 dark:text-cyan-400 rounded-xl">
              <List size={18} />
            </div>
            <div className="flex-1 flex justify-between items-center pr-1">
              <div>
                <span className="text-[10px] text-slate-404 font-bold block">Reviewed Questions</span>
                <span className="text-base font-black text-slate-800 dark:text-slate-100 font-sans">{reviewedCount}</span>
              </div>
              <div className="text-right">
                <span className="text-[10px] text-slate-404 font-bold block">Daily Avg</span>
          ...
          <div className="bg-slate-50 dark:bg-slate-955 p-4 rounded-2xl flex items-center gap-3 col-span-2">
            ...
            <div>
              <span className="text-[10px] text-slate-404 font-bold block">Completed Chapters</span>
              <span className="text-base font-black text-slate-800 dark:text-slate-100 font-sans">
                {completedChaptersCount} <span className="text-xs text-slate-404 font-medium">/ {allStats.length}</span>
              </span>
            </div>
          </div>
