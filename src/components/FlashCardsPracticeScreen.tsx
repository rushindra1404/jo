...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  'Cause-Effect':          'bg-teal-50 dark:bg-teal-950/40 text-teal-600 dark:text-teal-400 border-teal-100 dark:border-teal-800',
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800">
        ...
        {/* Card count + Quick Revision toggle */}
        <div className="px-4 py-1.5 flex items-center justify-between">
          <span className="text-[10px] font-black text-slate-500 dark:text-slate-400">
            {filteredCards.length > 0 ? `${currentIndex + 1} / ${filteredCards.length}` : '0 / 0'}
            {filteredCards.length > 0 && (
              <span className="ml-1 text-rose-500">{Math.round(progressPct)}%</span>
            )}
          </span>
          <button
            onClick={() => { setQuickRevision(q => !q); setIsFlipped(false); }}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border cursor-pointer active:scale-95 transition-all ${
              quickRevision
                ? 'bg-rose-500 text-white border-rose-500 shadow-sm'
                : 'bg-slate-50 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700'
            }`}
          >
            {quickRevision ? <><Pause size={9} /> Stop Auto</> : <><Play size={9} /> Quick Revision</>}
          </button>
        </div>
      </div>

      {/* ── Search + Filter bar (conditional) ────────────────────────────── */}
      <AnimatePresence>
        {showSearch && (
          <motion.div
            ...
            <div className="px-4 py-2 space-y-2">
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 rounded-xl px-3 py-2 border border-slate-200 dark:border-slate-700">
                ...
                <input
                  ...
                  autoFocus
                  className="flex-1 bg-transparent text-xs font-semibold text-slate-700 dark:text-slate-300 focus:outline-none placeholder-slate-400"
                />
                {searchQuery && (
              ...
              <div className="flex gap-1.5 flex-wrap">
                ...
                {(searchQuery || importanceFilter !== 'All') && (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border bg-red-50 dark:bg-red-950/30 text-red-500 border-red-200 dark:border-red-800 cursor-pointer"
                  >
      ...
      <div className="flex-1 px-4 py-4 flex flex-col items-center justify-center overflow-hidden">
        {filteredCards.length === 0 ? (
          <div className="text-center p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-xs shadow-sm space-y-3">
            ...
            <p className="text-xs font-black text-slate-500">No cards match your filter.</p>
            <button
              onClick={() => { setSearchQuery(''); setImportanceFilter('All'); }}
          ...
          activeCard && (
            <div
              className="w-full"
              style={{ perspective: '1200px', height: 'min(72svh, 460px)' }}
              onTouchStart={onTouchStart}
              onTouchEnd={onTouchEnd}
            >
              <AnimatePresence mode="wait">
                <motion.div
                  key={activeCard.uniqueId + (isFlipped ? '-back' : '-front')}
                ...
                >
                  {!isFlipped ? (
                    ...
                    <div
                      onClick={handleFlip}
                      className="w-full h-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-5 flex flex-col cursor-pointer select-none"
                    >
                      ...
                      <p className="text-center text-[8px] text-slate-300 dark:text-slate-700 font-medium">Swipe left / right to navigate</p>
                    </div>
                  ) : (
                    /* ─── BACK FACE ──────────────────────────────────────── */
                    <div
                      onClick={handleFlip}
                      className="w-full h-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-5 flex flex-col cursor-pointer select-none overflow-y-auto"
                    >
                      {/* Top row */}
                      ...
                      {/* Key Point box */}
                      <div className="mt-3 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-100 dark:border-slate-700 shrink-0">
                      ...
                      {/* Example (if present) */}
                      {activeCard.example && (
                        <div className="mt-3 p-3.5 bg-blue-50 dark:bg-blue-950/30 rounded-2xl border border-blue-100 dark:border-blue-900 shrink-0">
                      ...
                      {/* Notes (if present) */}
                      {activeCard.notes && (
                        <div className="mt-3 p-3.5 bg-amber-50 dark:bg-amber-950/30 rounded-2xl border border-amber-100 dark:border-amber-900 shrink-0">
                          <p className="text-[9px] font-black text-amber-500 uppercase tracking-wider mb-1">💡 Memory Tip</p>
                      ...
                      <p className="text-center text-[8px] text-slate-300 dark:text-slate-700 font-medium mt-3 shrink-0">
                        Tap to go back
                      </p>
                    </div>
                  )}
